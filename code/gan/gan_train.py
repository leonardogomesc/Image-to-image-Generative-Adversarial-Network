    ...
    for dset in datasets:
        for mtrl in materials:
            try:
                ...
                # Discriminator
                # netD = DISCRIMINATOR()
                # netD = MobileNetV3Small(height=224, width=224, classes=1)
                

                # UNet - Based Discriminator
                # netD = UNetDiscriminator(in_channels=1, height=64, width=64, classes=1)
                ...
                for epoch in range(EPOCHS):
                    ...
                    for i, data in enumerate(train_loader):
                        ...
                        # If using DCGAN
                        noise = torch.randn(real.size(0), nz, 1, 1).to(device)
                        fake = netG(noise)
